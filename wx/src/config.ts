 ...
 * 加载工作区配置（Vite 风格）。
 *
 * 在 cwd 或 --config 指定路径查找 termcorr.config.{mjs,js,cjs,json}。
 * 相对路径相对于配置文件目录解析。
 * outDir 若落在本 CLI 仓库内会报错，避免产物污染工具目录。
 */
...
export async function loadUserConfig(opts: LoadUserConfigOptions): Promise<ResolvedConfig> {
  ...
  if (isInsideDir(PKG_ROOT, outDir)) {
    throw new Error(
      `outDir 不能落在 CLI 工具目录内（${PKG_ROOT}）。请换到工作目录，或把 outDir 设为该目录下的 outputs。`,
    );
  }
