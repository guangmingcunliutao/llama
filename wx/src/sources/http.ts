...
export class HttpSearchSource implements SearchSource {
  ...
  async search(keyword: string): Promise<SourceDocument[]> {
    const cached = this.readCache(keyword);
    if (cached) return cached;

    await this.limiter.acquire(`source=${this.name} keyword=${keyword}`);

    const vars = { keyword, keyword_enc: encodeURIComponent(keyword) };
    ...
    if (method === "POST") {
      init.body = JSON.stringify(interpolate(this.options.body ?? {}, vars));
    }
