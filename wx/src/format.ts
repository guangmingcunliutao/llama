...
export function parseFormats(raw: unknown, fallback: SftFormat[] = ["alpaca"]): SftFormat[] {
  let tokens: string[] = [];
  ...
  for (const token of tokens) {
    ...
    if (!value) continue;
    if (value !== "alpaca" && value !== "sharegpt") {
      throw new Error(`未知 SFT 格式 ${value}，可选 alpaca / sharegpt`);
    }
