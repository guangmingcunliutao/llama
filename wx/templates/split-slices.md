...
```bash
termcorr split
termcorr suite          # split + 规则基线 infer + evaluate
termcorr infer --all    # 对四个 eval 文件各推理
termcorr evaluate --all
```

LlamaFactory 训练后可用 `termcorr analyze` 读 predict 产物；业务切片需分别改 yaml 里的 `eval_dataset` 再 predict。
